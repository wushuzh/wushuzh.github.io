...
> 有人在 SO 上问为什么软件的 32 bit 版本用 x86 标识，64 bit 版本用 x64 标识。Daniel C. Sobral 回顾的[历史渊源](https://serverfault.com/a/188203)大概如下:

> 早先 IBM PC 使用 16 bit CPU ，型号包括 8086、8088、80186、80286 , 后来 Intel 开发出 32 bit 兼容型的 CPU : 80386 它可以兼容运行先前为 16 bit CPU 开发的软件，既然和之前的都兼容，大家就将这些 CPU 统称 80x86，这次“瞻前顾后"的产品让 Intel 获得巨大成功，随后继续推出性能不断攀升的产品, 当市场普遍接受了产品，兼容性不成问题的时候索性把 ~~80~~ 去掉，直接将新产品叫做 ~~80~~486(我的第一台为了学习买的电脑)、~~80~~586 ...

> Intel 终于开发了 64 bit CPU，命名为 IA64，并同时将 "x86" 叫做 IA32。然而它大概了是忘了之前是怎么成功的。这次 IA64 居然无法兼容 IA32 或者叫做 x86，结果大家都不买。这时 AMD 适时地推出的可以兼容 x86 的 64 bit CPU，让市场易于理解其兼容性，推广时称其为 x86-64 , 果然大获成功。Intel 只能赶紧补课。再后兼容性已经不是个事了。于是又把 86 省略，即 ~~86~~x64

> 这种 承上启下的创新 -> 市场接受成为爆款 -> 抛开历史的创新 -> 市场漠视被人赶超 的循环似乎到处都有
